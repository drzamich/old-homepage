...
document.write(
  new Array(
    ...
    '<h4>Globalne zerowanie marginesów</h4><p>W języku <code>(X)HTML</code> niektóre elementy mają nadpisane marginesy zewnętrzne jak i wewnetrzne - różne w poszczególnych przeglądarkach. Kiedy robimy stronę internetową, po wstawieniu danego elementu posiadającega taki właśnie nadpisany margines, strona może nam się nieoczekiwanie "rozjechać".</p> <a href="porady-css.php">czytaj dalej</a>',

    '<h4>Kolejnośc wpisywania atrybutów mieszanych</h4><p>W CSS przy niektórych znacznikach mamy możliwość wpisywania kilku wartości do jednej cechy. Część z nich (na przykład <code>margin</code>) ma ustaloną kolejność wpisywania. Przy niektórych można jednak wpisywać wartości jak tylko chcemy.</p> <a href="porady-css.php">czytaj dalej</a>',

    '<h4>Wartości w cudzysłowie</h4><p>Pisząc strony WWW bardzo często musimy wpisać w znaczniku jego atrybut taki jak np. style, class, czy border. Aby to zastosować trzeba zapisać to tak: <p class="opis" style="margin-left: 5px; font-size: 11px;">przestarzały przykład:</p><pre style="margin: 5px;">&lt;<var>znacznik</var> <var>atrybut</var>=<var>wartość</var>&gt;</pre></p> <a href="porady-html.php">czytaj dalej</a>',
    ...
    '<h4>Zmniejszanie cięzaru obrazka</h4><p>Najbardziej znanymi formatami grafiki komputerowej są formaty <code>bmp</code>, <code>gif</code> <code>jpg</code> i <code>png</code>. Dzielą się one na dwie grupy - ciężkie i lekkie. Do ciężakich należą <code>bmp</code> i <code>png</code>, a do lekkich <code>jpg</code> i <code>gif</code>.</p> <a href="porady-grafika.php">czytaj dalej</a>',

    '<h4>Masowe zmniejszanie ciężaru obrazków</h4><p>Wiesz już jak zmniejszyć wagę pojedynczeo obrazka. Co jednak gdy masz cały folder ciezkich obrazkow? Czy musisz po kolei zmniejszać wagę każdego? Na szczęścnie nie! Wytłumacze tutaj jak w jednej chwili zmniejszyć wagę dowolnej ilości obrazków na raz</p><a href="porady-grafika.php">czytaj dalej</a>',

    '<h4>Zmniejszanie wielkości obrazka</h4><p><p>Obrazki występują w najróżniejszych wymiarach. Czasami gdy jakiś obrazek jest zbyt duży, po wstawieniu na stronę przestawia inne elementy. Można go pomniejszyć "siłowo" w takie sposób:<pre style="margin: 5px;">&lt;img width="<var>szerokość</var>" height="<var>wysokość</var>" /&gt;</pre></p><a href="porady-grafika.php">czytaj dalej</a>',

    '<h4>Masowe zmniejszanie wielkości obrazków</h4><p>Tak jak wcześniej zmienialiśmy rozszeżenie wielku grafik na raz, możemy również zmniejszyć ich rozmiary jednym kliknięciem. Nie musimy dzięki temu marnowac naszego czasu na zmniejszanie obrazków jeden po drugim.</p><a href="porady-grafika.php">czytaj dalej</a>'
  )
    .random()
    .join("")
);
